...
class TenantRepository {
  ...
  async resetPool (connectionString, dbID) {
    this.currentDb = dbID;
    const filenames = await fileReader.getModelFiles(this.modelsFolder, this.sharedModels);
    const result = await this.attachDbContext(filenames, connectionString, dbID);
    return result;
  }
  ...
  async currentDB (data) {
    ...
    if ((data.toLowerCase() === process.env.TENANCY_DEFAULT_HOSTNAME.toLowerCase())) {
      this.currentDb = 'default';
    } else {
      await this.db['default'].sequelize.query(`select uuid from hostnames where fqdn = '${data}'`)
        .then((result) => {
          this.currentDb = result[0][0].uuid;
        });
    }

    const result = await this.db[this.currentDb].sequelize;
    return result;
  }
  ...
  async auditLog (entity, action, oldvalue) {
    try {
      ...
      if (this.currentDb.toLowerCase() === 'default') {
        return;
      }
      ...
      const result = await this.audit.create(audits);
      return result;
    } catch (e) {
      throw new RepositoryError('DB', 'Add entity to audit collection failed', e);
    }
  }
  ...
  async add (entity) {
    try {
      ...
      throw new RepositoryError('DB', 'Add entity to main collection failed', e);
    }
  }
  ...
  async execute (sqlCommand) {
    try {
      return await this.db[this.currentDb].sequelize.query(sqlCommand);
    } catch (e) {
