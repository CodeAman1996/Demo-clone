...
const modelsPath = path.resolve(fileContent.datastore.modelsfolder);
...
cli.tenancy.install = async () => {
  ...
  console.log('Attempting to create a tenants folder in the Migrations folder.');
  await cli.tenancy.executeCommand(`mkdir -p ${migrationPath}/tenants`);

  console.log('Moving existing migrations to the migrations/tenants folder.');
  await cli.tenancy.executeCommand(`mv ${migrationPath}/*.js ${migrationPath}/tenants`);

  console.log('Copying tenancy migration files to the migrations folder');
  await cli.tenancy.executeCommand(`cp node_modules/node-multi-tenant/dist/migrations/*.js ${migrationPath}/`);

  console.log('Copying tenancy model files to the models folder');
  await cli.tenancy.executeCommand(`cp node_modules/node-multi-tenant/dist/models/*.js ${modelsPath}/`);

  console.log('Attempting to drop existing database. Kindly ignore message if database does not exists');
...
cli.horizontalLine = () => {
  ...
  console.log('\x1b[34m%s\x1b[0m', line);
};
